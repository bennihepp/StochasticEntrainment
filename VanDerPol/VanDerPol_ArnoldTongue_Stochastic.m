...
Y = zeros(num_of_iterations, length(Omega));
Yabs = zeros(num_of_iterations, length(Omega));
PDmean = zeros(num_of_iterations, 1);
PDstd = zeros(num_of_iterations, 1);


% for l=1:num_of_iterations
parfor l=1:num_of_iterations
...
%     Omega = omega;
    ...
    PDmean(l) = mean_peak_distance;
    PDstd(l) = std_peak_distance;

end

Y = reshape(Y, [length(input_periods), length(input_amplitudes), length(Omega)]);
Yabs = reshape(Yabs, [length(input_periods), length(input_amplitudes), length(Omega)]);
PDmean = reshape(PDmean, [length(input_periods), length(input_amplitudes)]);
PDstd = reshape(PDstd, [length(input_periods), length(input_amplitudes)]);
